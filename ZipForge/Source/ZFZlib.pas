...
// ZLib 1.2.5 (modified by ComponentAce to add 64-bit stream support)
...
type
  ...
  TZStreamRec = packed record
    ...
    avail_in: NativeInt;     // number of bytes available at next_in
    total_in: UInt64;       // total nb of input bytes read so far

    next_out:  PByte;      // next output byte should be put here
    avail_out: NativeInt;    // remaining free space at next_out
    total_out: UInt64;      // total nb of bytes output so far
    ...
    data_type: NativeInt;       //  best guess about the data type: ascii or binary
...
procedure debugStr(c: PAnsiChar); cdecl;
...
const
  ...
  {$IFNDEF WIN64}_z_errmsg:{$ELSE}z_errmsg:{$ENDIF} array[0..9] of PAnsiChar = (
...
{$IFDEF WIN64}
  ...
  {$L zlib64\crc32.obj}
  {$L zlib64\gvmat64.obj}
  {$L zlib64\inffasx64.obj}
{$ELSE}
...
procedure debugStr(c: PAnsiChar);
begin
  //debugStr(c);
end;
