...
//            break;
          // added in 5.07, thanks to Craig Peterson 
          if (strm.avail_out > 0) and (strm.avail_in <= 0) then
             break;
          ...
          {$IFDEF WIN64}
          strm.next_out  := PByte(NativeInt(Buf)); // + strm.total_out - oldSize);
          {$ELSE}
  ...
  try
    ...
    begin
      ...
      {$IFDEF ZF_VERSION}
        ...
        else if (not FEncrypted) or (FEncrypted and (d.ZFEncryptionAlgorithm = caPkzipClassic)) then
        begin
