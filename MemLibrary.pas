...
function memLoadLibrary(fp_data: Pointer; const f_size: int64;
  f_SectionFinalization: TSFFlag = SF_BOTH;
  f_DllProcessAttach: Boolean = True): PBTMemoryModule;

// return value is nil if function fails
function memGetProcAddress(fp_module: PBTMemoryModule;
  const fp_name: PChar): Pointer;
// free module
procedure memFreeLibrary(fp_module: PBTMemoryModule);
// returns last error
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        lastErrStr :=
          (
          'BuildImportTable: bu neng load library: ' +
          PChar(@s_temp[0])
          );
        Result := false;
        exit;
      ...
      if fp_module^.modules = nil then
      begin
        lastErrStr := 'BuildImportTable zhong ReallocMemory shi bai';
        Result := false;
        exit;
      end;
      ...
      begin
        ...
        begin
          lastErrStr := 'BuildImportTable zhong GetProcAddress shi bai';
          Result := false;
          break;
        end;
        Inc(lp_funcRef);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            lastErrStr := 'FinalizeSections zhong  VirtualProtect shi bai';
            exit;
          end;
        end;
      end;
...
begin
  ...
  try
    ...
    begin
      lastErrStr := 'memLoadLibary zhong dll dos header bu ke yong';
      exit;
    end
    else
    ...
    begin
      lastErrStr := 'memLoadLibary zhong IMAGE_NT_SIGNATURE bu ke yong';
      exit;
    end
    else
    ...
    if l_code = nil then
    begin
      lastErrStr := 'memLoadLibary zhong  VirtualAlloc shi bai';
      exit;
    end
    else
    ...
    begin
      lastErrStr := lastErrStr + ' memLoadLibary: BuildImportTable shi bai';
      Abort;
    end;
    ...
    begin
      ...
      begin
        lastErrStr := 'memLoadLibary zhong  huo qu DLLEntyPoint shi bai';
        Abort;
      end;
      if f_DllProcessAttach then begin
        ...
        begin
          lastErrStr := 'memLoadLibary zhong bu neng  attach library';
          Abort;
        end;
      end;
...
begin
  ...
  begin
    lastErrStr := 'memGetProcAddress zhong  no export table zhao dao';
    exit;
  end;
  ...
  begin
    lastErrStr := 'memGetProcAddress zhong  DLL bu neng  dao chu  anything';
    exit;
  end;
  ...
  if (l_idx = -1) then
  begin
    lastErrStr := 'memGetProcAddress zhong  dao chu ge shi mei zhao dao';
    exit;
  end;
  if (UInt64(l_idx) > l_exports^.NumberOfFunctions - 1) then
  begin
    lastErrStr :=
      'memGetProcAddress zhong  dan qian ming zhi he yuan lai bu qi pei';
    exit;
  end;
